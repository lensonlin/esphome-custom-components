...
static const uint8_t BL0910_FREQUENCY = 0X4E;     //线电压频率周期寄存器，无符号
static const uint8_t BL0910_TEMPERATURE = 0X5E;     //内部温度值寄存器，无符号  
//校正寄存器
...
static const uint8_t BL0910_RMSGN_11 = 0x76; //有效值增益调整寄存器地址,通道11
...
static const uint8_t BL0910_RMSOS_11 = 0x81; //有效值偏置校正寄存器地址,通道11 
...
//Bias_correction(BL0910_RMSOS_1, 0.01960, 0);//电流有效值偏置校正，通道1
//Bias_correction(BL0910_RMSOS_2, 0.02673, 0);//电流有效值偏置校正，通道2
//Bias_correction(BL0910_RMSOS_3, 0.02895, 0);//电流有效值偏置校正，通道3
//Bias_correction(BL0910_RMSOS_4, 0.03604, 0);//电流有效值偏置校正，通道4
//Bias_correction(BL0910_RMSOS_5, 0.03056, 0);//电流有效值偏置校正，通道5
//Bias_correction(BL0910_RMSOS_6, 0.02949, 0);//电流有效值偏置校正，通道6
//Bias_correction(BL0910_RMSOS_7, 0.02895, 0);//电流有效值偏置校正，通道7
//Bias_correction(BL0910_RMSOS_8, 0.03604, 0);//电流有效值偏置校正，通道8
//Bias_correction(BL0910_RMSOS_9, 0.03056, 0);//电流有效值偏置校正，通道9
//Bias_correction(BL0910_RMSOS_10, 0.02949, 0);//电流有效值偏置校正，通道10
//gain_correction(BL0910_RMSGN_1, 0.07169, 0.068, BL0910_ki);//电流有效值增益调整，通道1
//gain_correction(BL0910_RMSGN_2, 0.07232, 0.068, BL0910_ki);//电流有效值增益调整，通道2
//gain_correction(BL0910_RMSGN_3, 0.07333, 0.068, BL0910_ki);//电流有效值增益调整，通道3
//gain_correction(BL0910_RMSGN_4, 0.07779, 0.068, BL0910_ki);//电流有效值增益调整，通道4
//gain_correction(BL0910_RMSGN_5, 0.07224, 0.068, BL0910_ki);//电流有效值增益调整，通道5
//gain_correction(BL0910_RMSGN_6, 0.14988, 0.068, BL0910_ki);//电流有效值增益调整，通道6
//gain_correction(BL0910_RMSGN_7, 0.07333, 0.068, BL0910_ki);//电流有效值增益调整，通道7
//gain_correction(BL0910_RMSGN_8, 0.07779, 0.068, BL0910_ki);//电流有效值增益调整，通道8
//gain_correction(BL0910_RMSGN_9, 0.07224, 0.068, BL0910_ki);//电流有效值增益调整，通道9
//gain_correction(BL0910_RMSGN_10, 0.14988, 0.068, BL0910_ki);//电流有效值增益调整，通道10
...
//gain_correction(BL0910_WATTGN_7, 15.44956, 14.5, BL0910_Kp);//功率增益调整，通道7
//gain_correction(BL0910_WATTGN_8, 16.57646, 14.5, BL0910_Kp);//功率增益调整，通道8
//gain_correction(BL0910_WATTGN_9, 15.27440, 14.5, BL0910_Kp);//功率增益调整，通道9
...
void BL0910::update() {
  ...
  read_data(BL0910_I_3_RMS, current_reference_, current_sensor_7_ ); //电流,通道7
  read_data(BL0910_I_4_RMS, current_reference_, current_sensor_8_ ); //电流,通道8
  read_data(BL0910_I_5_RMS, current_reference_, current_sensor_9_ ); //电流,通道9
  read_data(BL0910_I_6_RMS, current_reference_, current_sensor_10_ ); //电流,通道10
  ...
  read_data(BL0910_WATT_3, power_reference_, power_sensor_7_ );//功率。通道7
  read_data(BL0910_WATT_4, power_reference_, power_sensor_8_ );//功率。通道8 
  read_data(BL0910_WATT_5, power_reference_, power_sensor_9_ );//功率。通道9 
  read_data(BL0910_WATT_6, power_reference_, power_sensor_10_ );//功率。通道10 
  read_data(BL0910_WATT_SUM, power_reference_, power_sensor_sum_ );//功率。通道总和 
  ...
  read_data(BL0910_CF_3_CNT, energy_reference_, energy_sensor_7_ ); //电量。通道7
  read_data(BL0910_CF_4_CNT, energy_reference_, energy_sensor_8_ ); //电量。通道8
  read_data(BL0910_CF_5_CNT, energy_reference_, energy_sensor_9_ ); //电量。通道9
  read_data(BL0910_CF_6_CNT, energy_reference_, energy_sensor_10_ ); //电量。通道10
  read_data(BL0910_CF_SUM_CNT, energy_reference_, energy_sensor_sum_ ); //电量。通道总和
}
